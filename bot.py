        ...
        for event in longpoll.listen():
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                # ========== НАКАЗАНИЯ ==========
                    ...
                    parts = text.split()
                    if len(parts) < 2:
                        vk.messages.send(peer_id=peer_id, message="❌ Пример: /mute 10м @пользователь причина", random_id=random.getrandbits(64), reply_to=msg_id)
                        continue
                    ...
                    if not target_id:
                        vk.messages.send(peer_id=peer_id, message="❌ Укажи пользователя: /mute 10м @пользователь", random_id=random.getrandbits(64), reply_to=msg_id)
                        continue
                    ...
                    duration_str = format_time(time_seconds)
                    reason_text = reason if reason else "не указана"
                    vk.messages.send(peer_id=peer_id, message=f"🔇 МУТ | {admin_name} выдал блокировку чата пользователю - {target_name} на {duration_str} по причине: {reason_text}", random_id=random.getrandbits(64), reply_to=msg_id)
                    ...
                    if not target_id:
                        vk.messages.send(peer_id=peer_id, message="❌ Укажи пользователя: /unmute @пользователь", random_id=random.getrandbits(64), reply_to=msg_id)
                        continue
                    
                    update_user(target_id, peer_id, "muted_until", 0)
                    admin_name = get_display_name(user_id, peer_id)
                    target_name = get_display_name(target_id, peer_id)
                    vk.messages.send(peer_id=peer_id, message=f"🔊 РАЗМУТ | {admin_name} снял блокировку чата с пользователя - {target_name}", random_id=random.getrandbits(64), reply_to=msg_id)
                    ...
                    if new_warns >= 3:
                        try:
                            ...
                            vk.messages.send(peer_id=peer_id, message=f"⚠️ ВАРН | {admin_name} выдал 3/3 варнов пользователю - {target_name} по причине: {reason_text}\n\n🔴 Пользователь кикнут из беседы!", random_id=random.getrandbits(64), reply_to=msg_id)
                        except Exception as e:
                            vk.messages.send(peer_id=peer_id, message=f"⚠️ ВАРН | {admin_name} выдал 3/3 варнов пользователю - {target_name} по причине: {reason_text}\n\n⚠️ Не удалось кикнуть (дайте права админа)", random_id=random.getrandbits(64), reply_to=msg_id)
                    else:
                        update_user(target_id, peer_id, "warns", new_warns)
                        vk.messages.send(peer_id=peer_id, message=f"⚠️ ВАРН | {admin_name} выдал предупреждение пользователю - {target_name} по причине: {reason_text}\n\n📊 Всего варнов: {new_warns}/3", random_id=random.getrandbits(64), reply_to=msg_id)
                    ...
                    try:
                        ...
                        target_name = get_display_name(target_id, peer_id)
                        reason_text = reason if reason else "не указана"
                        vk.messages.send(peer_id=peer_id, message=f"💥 КИК | {admin_name} исключил пользователя - {target_name} по причине: {reason_text}", random_id=random.getrandbits(64), reply_to=msg_id)
                    except Exception as e:
                    ...
                    try:
                        ...
                        target_name = get_display_name(target_id, peer_id)
                        reason_text = reason if reason else "не указана"
                        vk.messages.send(peer_id=peer_id, message=f"⛔ БАН | {admin_name} заблокировал пользователя - {target_name} по причине: {reason_text}\n\n🚫 Пользователь добавлен в чёрный список группы!", random_id=random.getrandbits(64), reply_to=msg_id)
                        try:
                            chat_id = peer_id - 2000000000
                            vk.messages.removeChatUser(chat_id=chat_id, user_id=target_id)
                        except:
                    ...
                    try:
                        ...
                        if not is_banned:
                            vk.messages.send(peer_id=peer_id, message=f"❌ Пользователь не в чёрном списке!", random_id=random.getrandbits(64), reply_to=msg_id)
                            continue
                        ...
                        vk.messages.send(peer_id=peer_id, message=f"🔓 РАЗБАН | {admin_name} разблокировал пользователя - {target_name}{reason_text}", random_id=random.getrandbits(64), reply_to=msg_id)
                    except Exception as e:
                        vk.messages.send(peer_id=peer_id, message=f"❌ Ошибка: {e}", random_id=random.getrandbits(64), reply_to=msg_id)
                ...
                # ========== КАСТОМНЫЕ КОМАНДЫ ==========
                    ...
                    builtin_commands = ["help", "stats", "work", "casino", "staff", "get", "mute", "unmute", 
                                       "warn", "warnlist", "unwarn", "kick", "ban", "unban", "banlist", "setrank", "unrank",
                                       "duel", "accept", "nick", "delnick", "linkchat", "unlinkchat", "chats", "gban", "gunban", 
                                       "gkick", "gsetrank", "gdelrank", "cmd", "delcmd", "cmdlist", "top", "bonus", "leagues", 
                                       "quiet", "ready", "add_xp", "remove_xp", "set_level", "reset_user", "reset_chat", "dev_help",
                ...
                # ========== КОМАНДЫ РАЗРАБОТЧИКА ==========
                    ...
                    dev_name = get_display_name(user_id, peer_id)
                    target_name = get_display_name(target_id, peer_id)
                    vk.messages.send(peer_id=peer_id, message=f"📉 ЗАБОР XP | Разработчик {dev_name} забрал у {target_name} -{amount} XP!", random_id=random.getrandbits(64), reply_to=msg_id)
                    ...
                    dev_name = get_display_name(user_id, peer_id)
                    target_name = get_display_name(target_id, peer_id)
                    vk.messages.send(peer_id=peer_id, message=f"📊 УСТАНОВКА УРОВНЯ | Разработчик {dev_name} установил {target_name} {new_level} уровень!", random_id=random.getrandbits(64), reply_to=msg_id)
                    ...
                    dev_name = get_display_name(user_id, peer_id)
                    target_name = get_display_name(target_id, peer_id)
                    vk.messages.send(peer_id=peer_id, message=f"🔄 СБРОС | Разработчик {dev_name} сбросил все данные пользователя {target_name}!", random_id=random.getrandbits(64), reply_to=msg_id)
                    ...
                    conn = sqlite3.connect("hold_manager_v2.db")
                    cursor = conn.cursor()
                    cursor.execute("UPDATE users SET xp = 0, warns = 0, duels_won = 0, duels_lost = 0, nickname = '', muted_until = 0 WHERE peer_id = ?", (peer_id,))
                    conn.commit()
                    conn.close()
                    
                    dev_name = get_display_name(user_id, peer_id)
                    vk.messages.send(peer_id=peer_id, message=f"🔄 СБРОС ЧАТА | Разработчик {dev_name} сбросил все данные в этой беседе!", random_id=random.getrandbits(64), reply_to=msg_id)
                    ...
                    help_text = (
                        "🔧 КОМАНДЫ РАЗРАБОТЧИКА 🔧\n\n"
                        "/add_xp <количество> @пользователь — выдать XP\n"
                        "/remove_xp <количество> @пользователь — забрать XP\n"
                        ...
                        "/create_promo <XP> [макс.использований] — создать промокод\n"
                ...
                # ========== ПРОМОКОДЫ ==========
                    ...
                    if not promos:
                        vk.messages.send(peer_id=peer_id, message="📋 Нет созданных промокодов.\n\nСоздай: /create_promo 1000", random_id=random.getrandbits(64), reply_to=msg_id)
                        continue
                    
                    promo_text = "📋 СПИСОК ПРОМОКОДОВ\n\n"
                    for code, xp_reward, created_at, max_uses, uses in promos:
                        promo_text += f"🎫 {code}\n✨ {xp_reward} XP\n📊 Использований: {uses}/{max_uses}\n\n"
                ...
                # ========== ГЛОБАЛЬНЫЕ КОМАНДЫ ==========
                    ...
                    admin_name = get_display_name(user_id, peer_id)
                    target_name = get_display_name(target_id, peer_id)
                    vk.messages.send(peer_id=peer_id, message=f"⛔ ГЛОБАЛЬНЫЙ БАН | {admin_name} заблокировал {target_name} в {banned_count} беседах{reason_text}", random_id=random.getrandbits(64), reply_to=msg_id)
                    ...
                    admin_name = get_display_name(user_id, peer_id)
                    target_name = get_display_name(target_id, peer_id)
                    vk.messages.send(peer_id=peer_id, message=f"🔓 ГЛОБАЛЬНЫЙ РАЗБАН | {admin_name} разблокировал {target_name} в {unbanned_count} беседах{reason_text}", random_id=random.getrandbits(64), reply_to=msg_id)
                    ...
                    admin_name = get_display_name(user_id, peer_id)
                    target_name = get_display_name(target_id, peer_id)
                    vk.messages.send(peer_id=peer_id, message=f"💥 ГЛОБАЛЬНЫЙ КИК | {admin_name} исключил {target_name} из {kicked_count} бесед{reason_text}", random_id=random.getrandbits(64), reply_to=msg_id)
                    ...
                    admin_name = get_display_name(user_id, peer_id)
                    target_name = get_display_name(target_id, peer_id)
                    vk.messages.send(peer_id=peer_id, message=f"👑 ГЛОБАЛЬНАЯ ВЫДАЧА РАНГА | {admin_name} выдал {target_name} ранг {rank_name} в {success_count} беседах!", random_id=random.getrandbits(64), reply_to=msg_id)
                    ...
                    admin_name = get_display_name(user_id, peer_id)
                    target_name = get_display_name(target_id, peer_id)
                    vk.messages.send(peer_id=peer_id, message=f"❌ ГЛОБАЛЬНОЕ СНЯТИЕ РАНГА | {admin_name} снял ранг с {target_name} в {success_count} беседах!", random_id=random.getrandbits(64), reply_to=msg_id)
                ...
                # ========== СПРАВКА ==========
                    ...
                    help_text = (
                        ...
                        "🔧 КОМАНДЫ РАЗРАБОТЧИКА\n"
                        "/add_xp, /remove_xp, /set_level, /reset_user, /reset_chat\n"
                        "/create_promo, /use_promo, /promos, /dev_help"
                    )
                    vk.messages.send(peer_id=peer_id, message=help_text, random_id=random.getrandbits(64), reply_to=msg_id)
