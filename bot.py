...
def create_stats_card(user_id, peer_id, user_data):
    """Создаёт уникальную карточку статистики"""
    ...
    for i in range(height):
        r = 26 + int(i / height * 30)
        g = 26 + int(i / height * 40)
        b = 46 + int(i / height * 50)
        draw.line([(0, i), (width, i)], fill=(r, g, b))
    
    # Декоративные линии
    draw.rectangle([(5, 5), (width-5, height-5)], outline=accent_color, width=3)
    draw.rectangle([(10, 10), (width-10, height-10)], outline=accent_color, width=1)
    ...
    draw.ellipse([(avatar_x, avatar_y), (avatar_x + avatar_size, avatar_y + avatar_size)], 
                 fill='#e94560', outline='#ffffff', width=3)
    ...
    draw.text((avatar_x + (avatar_size - text_w) // 2, avatar_y + (avatar_size - text_h) // 2), 
              initials, fill='white', font=font_large)
    ...
    draw.text((avatar_x + avatar_size + 20, avatar_y + 45), f"⭐ {rank_name}", fill=accent_color, font=font_name)
    
    # Разделитель
    draw.line([(20, 120), (width-20, 120)], fill=accent_color, width=2)
    ...
    draw.text((30, 140), "УРОВЕНЬ", fill='#888888', font=font_label)
    draw.text((30, 165), str(user_level), fill=accent_color, font=font_value)
    
    draw.text((30, 210), "ОПЫТ", fill='#888888', font=font_label)
    draw.text((30, 235), f"{user_data['xp']:,}", fill='white', font=font_small)
    
    # Правая колонка
    draw.text((260, 140), "ЛИГА", fill='#888888', font=font_label)
    draw.text((260, 165), league_name, fill=league_color, font=font_value)
    
    draw.text((260, 210), "ДУЭЛИ", fill='#888888', font=font_label)
    draw.text((260, 235), f"🏆 {user_data['duels_won']} / {user_data['duels_lost']}", fill='white', font=font_small)
    
    # Нижняя часть
    draw.line([(20, 280), (width-20, 280)], fill=accent_color, width=1)
    
    # Варны
    warn_color = '#ff5555' if user_data["warns"] > 0 else '#88ff88'
    draw.text((30, 295), f"⚠️ ВАРНЫ: {user_data['warns']}/3", fill=warn_color, font=font_small)
    
    # Мут
    is_muted = user_data["muted_until"] > int(time.time())
    if is_muted:
        mute_left = user_data["muted_until"] - int(time.time())
        draw.text((260, 295), f"🔇 МУТ: {format_time(mute_left)}", fill='#ff8888', font=font_small)
    else:
        draw.text((260, 295), "🔊 МУТ: НЕТ", fill='#88ff88', font=font_small)
    
    # Дата
    current_date = time.strftime("%d.%m.%Y", time.localtime())
    draw.text((30, 335), f"📅 {current_date}", fill='#666666', font=font_label)
    
    # Подпись бота
    draw.text((width - 120, 335), "HOLD MANAGER", fill='#e94560', font=font_label)
    ...
    try:
        for event in longpoll.listen():
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                # ========== ОСТАЛЬНЫЕ КОМАНДЫ (сокращённо из-за лимита символов) ==========
                elif text == "/ready":
                ...
                # ========== ОСТАЛЬНЫЕ КОМАНДЫ (для краткости оставлены основные) ==========
                # /get, /mute, /warn, /kick, /ban, /setrank, /quiet и др.
                # Они аналогичны предыдущим версиям, добавь их при необходимости
