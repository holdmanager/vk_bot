...
def create_stats_card(user_id, peer_id, user_data):
    """Создаёт неоновую карточку статистики с аватаркой"""
    ...
    for i in range(5):
        ...
        draw.rectangle([(offset, offset), (width-offset, height-offset)], 
                       outline=f"#{'ff2a6d' if i % 2 == 0 else '05d9e8'}", width=1)
    
    # Углы
    corner_size = 20
    draw.line([(10, corner_size), (10, 10), (corner_size, 10)], fill=neon_pink, width=3)
    draw.line([(width-10, corner_size), (width-10, 10), (width-corner_size, 10)], fill=neon_blue, width=3)
    draw.line([(10, height-corner_size), (10, height-10), (corner_size, height-10)], fill=neon_blue, width=3)
    draw.line([(width-10, height-corner_size), (width-10, height-10), (width-corner_size, height-10)], fill=neon_pink, width=3)
    
    # Загружаем аватарку
    avatar_url = get_user_avatar_url(user_id)
    avatar_img = None
    avatar_size = 90
    avatar_x, avatar_y = 25, 25
    ...
    if avatar_img:
        ...
        # Применяем маску
        temp = Image.new('RGB', (avatar_size, avatar_size), dark_bg)
        temp.paste(avatar_img, (0, 0))
        temp.putalpha(mask)
        
        img.paste(temp, (avatar_x, avatar_y), temp)
        
        # Неоновая подсветка аватара
        draw.ellipse([(avatar_x-2, avatar_y-2), (avatar_x+avatar_size+2, avatar_y+avatar_size+2)], 
                     outline=neon_pink, width=2)
        draw.ellipse([(avatar_x-1, avatar_y-1), (avatar_x+avatar_size+1, avatar_y+avatar_size+1)], 
                     outline=neon_blue, width=1)
    else:
        # Если аватарка не загрузилась — инициалы
        name = get_display_name(user_id, peer_id)
        initials = ''.join([word[0].upper() for word in name.split()[:2]])
        
        draw.ellipse([(avatar_x, avatar_y), (avatar_x + avatar_size, avatar_y + avatar_size)], 
                     fill='#1a1a2e', outline=neon_pink, width=3)
        draw.ellipse([(avatar_x+2, avatar_y+2), (avatar_x+avatar_size-2, avatar_y+avatar_size-2)], 
                     outline=neon_blue, width=1)
        
        font_initials = get_font(34)
        ...
        draw.text((avatar_x + (avatar_size - text_w) // 2, avatar_y + (avatar_size - text_h) // 2 - 5), 
                  initials, fill=neon_pink, font=font_initials)
    ...
    draw.text((avatar_x + avatar_size + 20, avatar_y + 15), name, fill='#ffffff', font=font_name)
    
    # Ранг с неоновым эффектом
    rank_name = RANGS.get(user_data["role"], "Пользователь")
    font_rank = get_font(15)
    draw.text((avatar_x + avatar_size + 20, avatar_y + 50), f"[ {rank_name} ]", fill=neon_blue, font=font_rank)
    
    # Разделитель
    draw.line([(20, 135), (width-20, 135)], fill=neon_pink, width=1)
    draw.line([(20, 136), (width-20, 136)], fill=neon_blue, width=1)
    ...
    league_color = league['color'] if league else '#888888'
    ...
    # Левая колонка
    draw.text((35, 155), "УРОВЕНЬ", fill='#666666', font=font_label)
    draw.text((35, 178), str(user_level), fill=neon_pink, font=font_value)
    
    draw.text((35, 225), "ОПЫТ", fill='#666666', font=font_label)
    draw.text((35, 248), f"{user_data['xp']:,}", fill='#ffffff', font=font_small)
    
    # Центральная колонка
    draw.text((190, 155), "ЛИГА", fill='#666666', font=font_label)
    draw.text((190, 178), league_name, fill=league_color, font=font_small)
    
    draw.text((190, 225), "ДУЭЛИ", fill='#666666', font=font_label)
    draw.text((190, 248), f"🏆 {user_data['duels_won']} / {user_data['duels_lost']}", fill='#ffffff', font=font_small)
    
    # Правая колонка
    draw.text((345, 155), "ВАРНЫ", fill='#666666', font=font_label)
    warn_color = neon_pink if user_data["warns"] > 0 else '#88ff88'
    draw.text((345, 178), f"{user_data['warns']}/3", fill=warn_color, font=font_small)
    
    draw.text((345, 225), "МУТ", fill='#666666', font=font_label)
    is_muted = user_data["muted_until"] > int(time.time())
    if is_muted:
        mute_left = user_data["muted_until"] - int(time.time())
        draw.text((345, 248), f"{format_time(mute_left)}", fill='#ff8888', font=font_small)
    else:
        draw.text((345, 248), "НЕТ", fill='#88ff88', font=font_small)
    
    # Прогресс-бар уровня
    current_level_xp = user_level * 1000
    xp_progress = user_data["xp"] - current_level_xp
    progress_percent = (xp_progress / 1000) * 100 if user_data["xp"] > 0 else 0
    ...
    draw.rectangle([(bar_x, bar_y), (bar_x + bar_width, bar_y + bar_height)], fill='#1a1a2e', outline=neon_blue, width=1)
    if filled_width > 0:
        draw.rectangle([(bar_x, bar_y), (bar_x + filled_width, bar_y + bar_height)], fill=neon_pink)
    
    xp_for_next = (user_level + 1) * 1000 - user_data["xp"]
    draw.text((bar_x + 5, bar_y - 18), f"ДО СЛЕДУЮЩЕГО УРОВНЯ: {xp_for_next} XP", fill='#888888', font=get_font(10))
    
    # Нижняя линия
    draw.line([(20, 320), (width-20, 320)], fill=neon_blue, width=1)
    draw.line([(20, 321), (width-20, 321)], fill=neon_pink, width=1)
    
    # Дата
    current_date = time.strftime("%d.%m.%Y", time.localtime())
    draw.text((30, 340), current_date, fill='#666666', font=font_label)
    
    # Неоновая подпись
    draw.text((width - 130, 340), "HOLD MANAGER", fill=neon_pink, font=font_label)
    ...
    try:
        for event in longpoll.listen():
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                # ========== ОСТАЛЬНЫЕ АДМИН-КОМАНДЫ (шаблоны) ==========
                # /mute, /unmute, /warn, /unwarn, /warnlist, /kick, /ban, /unban, /banlist, /setrank, /unrank
                # /quiet, /linkchat, /unlinkchat, /chats, /gban, /gunban, /gkick, /gsetrank, /gdelrank
                # /cmd, /delcmd, /cmdlist, /add_xp, /remove_xp, /set_level, /reset_user, /reset_chat
                # /create_promo, /use_promo, /promos, /dev_help
                # Они аналогичны предыдущим версиям, добавь их при необходимости из старого кода
